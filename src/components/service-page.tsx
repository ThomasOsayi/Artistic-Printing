...
import {
  // Hero badge / service icons
  CreditCard, BookOpen, Image as ImageIcon, Megaphone, Printer, Layers,
  // Why-us / quality
  ShieldCheck, Sparkles, Zap, Truck, Award, BadgeCheck, Clock, Package,
  Palette, Leaf, Star, Type,
  // Spec + option items
  Ruler, Scissors, Stamp, PenTool, Frame, Monitor, Square, Flag, Tag,
  FileText, ScrollText, NotebookPen, IdCard, FolderOpen, Copy,
  ClipboardList, FileSignature, Calendar, Ticket, Gift, MapPin,
  // Trust logos
  Building, Building2, Users, Store, Hospital, Utensils, Car,
  GraduationCap, Briefcase, HeartPulse,
  // UI
  HelpCircle, ChevronDown, ArrowRight, Phone, Mail, CheckCircle2,
  type LucideIcon,
} from 'lucide-react'
import type { PageContent, GradientColor, AccentColor } from '@/lib/admin-data'

// ─── Icon name → component map ──────────────────────────────────────────
// PageContent docs store icon names as kebab-case strings; mapped here at
// render time. Add new icons as service seed data requires.
const ICONS: Record<string, LucideIcon> = {
  // Hero badges / service types
...
// ─── Tailwind class maps ────────────────────────────────────────────────
// Full literal strings so Tailwind's JIT sees them at build time.
...
const DEFAULT_PROCESS_STEPS = [
  { title: 'Send Your Specs',  description: 'Quantity, size, stock — or just tell us what it is for' },
  { title: 'Get a Quote',      description: 'Priced and back to you within 24 hours' },
  { title: 'Approve a Proof',  description: 'Digital proof until the color and layout are right' },
  { title: 'We Print It',      description: 'Digital or offset, with in-house finishing' },
...
export function ServicePage({ content, breadcrumbLabel }: ServicePageProps) {
  ...
  const turnaround    = s(content, 'turnaround')
  ...
  return (
    <div>
      ...
      <section className="relative py-20 sm:py-28 bg-slate-900 text-white overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          <div className="max-w-3xl">
            {content.heroBadge && (
              <div
                className={`inline-flex items-center gap-2 bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 px-3 py-1 rounded-full text-xs font-medium mb-6 transition-all duration-700 ${
            ...
            {turnaround && (
              ...
              >
                <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur border border-white/20 rounded-xl px-4 py-2.5">
                  <Clock className="w-4 h-4 text-cyan-400 flex-shrink-0" />
                  <div className="text-left leading-tight">
                    <div className="text-[10px] uppercase tracking-wider text-slate-400">Turnaround</div>
      ...
      {/* ═══════════ SPECS ═══════════ */}
      {content.specRows && content.specRows.length > 0 && (
        <section className="py-20 bg-slate-50" ref={specRef}>
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              {s(content, 'specEyebrow') && (
                <Badge data-reveal className="bg-emerald-100 text-emerald-700 mb-4">
                  {s(content, 'specEyebrow')}
                </Badge>
              )}
              {s(content, 'specHeading') && (
                <h2 data-reveal="delay-1" className="text-3xl sm:text-4xl font-bold text-slate-900 mb-4">
                  {s(content, 'specHeading')}
                </h2>
              )}
            ...
            >
              <dl>
                {content.specRows.map((row, i) => (
                  <div
                    key={i}
                    className={`grid sm:grid-cols-3 gap-1 sm:gap-6 px-5 sm:px-7 py-5 ${
                      i % 2 === 1 ? 'bg-slate-50/70' : 'bg-white'
                    } ${i !== 0 ? 'border-t border-slate-100' : ''}`}
                  >
                    <dt className="text-sm font-semibold text-slate-900 sm:col-span-1">
                      {row.label}
                    </dt>
                    <dd className="text-sm text-slate-600 leading-relaxed sm:col-span-2">
                      {row.value}
                    </dd>
                  </div>
                ))}
            ...
            <p data-reveal="delay-3" className="text-center text-sm text-slate-500 mt-6">
              Need something outside these specs?{' '}
              <a href="#quote" className="text-cyan-600 font-medium hover:text-cyan-700">
                Ask us — we quote custom jobs daily
              </a>
              .
            </p>
          </div>
      ...
      {/* ═══════════ OPTIONS / WHAT WE PRINT ═══════════ */}
      {content.printItems && content.printItems.length > 0 && (
        <section className="py-20 bg-white" ref={printRef}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid lg:grid-cols-3 gap-12 items-start mb-12">
              <div className="lg:col-span-1">
                ...
                >
                  Not sure which option fits? Ask us
